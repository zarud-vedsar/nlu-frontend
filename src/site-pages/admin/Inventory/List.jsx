...
function TimeSlot() {
    ...
    const submitForm = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${NODE_API_URL}/api/inventory/category/create`, formData);
    ...
    const handleToggleStatus = async (dbId, currentStatus) => {
        ...
        try {
            ...
            const response = await dataFetchingPatch(`${NODE_API_URL}/api/inventory/category/status/${dbId}/${loguserid}/${login_type}`);
    ...
    const updateDataFetch = async (dbId) => {
        if (!dbId || (!Number.isInteger(parseInt(dbId, 10)) || parseInt(dbId, 10) <= 0)) return toast.error("Invalid ID.");
        try {
            const response = await dataFetchingPost(`${NODE_API_URL}/api/inventory/category/fetch`,
                { dbId }
            );
            if (response?.statusCode === 200 && response.data.length > 0) {
                toast.success(response.message);
                    ...
                    <div className="container-fluid">
                        ...
                        <div className="card bg-transparent mb-2">
                            <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                                <h5 className="card-title h6_new pt-2">Category List</h5>
                            ...
                            <div className="card-body">
                                ...
                                <div className={`table-responsive ${isFetching ? 'form' : ''}`}>
                                    <DataTable
                                        ...
                                        sortMode="multiple"
                                    >
                                        <Column field="ctitle" header="Title" sortable />
            ...
            <Modal show={toggleShow} onHide={handleToggleShow}>
                ...
                <Modal.Body>
                    <form onSubmit={(e) => e.preventDefault()}>
                        <div className='row'>
                            <FormField label="Title" name="ctitle" id="ctitle" borderError={titleError ? true : false} errorMessage={titleError} value={formData.ctitle} column='col-md-12' onChange={handleChange} />
                        </div>
                    </form>
                </Modal.Body>
                <Modal.Footer className='d-flex nowrap justify-content-between'>
                    ...
                    <Button variant="dark" className='d-flex justify-content-center align-items-center'
                        onClick={submitForm}>
                        Save{" "} {isSubmit && (
                            <>
