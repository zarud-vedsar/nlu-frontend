...
function FacultyDepartment() {
    ...
    const [facultyDepartmentList, setFacultyDepartmentList] = useState([]);
    ...
    const fetchList = async (deleteStatus = 0) => {
        setIsFetching(true);
        try {
            const response = await dataFetchingPost(`${NODE_API_URL}/api/faculty-department/fetch`, { deleteStatus });
            if (response?.statusCode === 200 && response.data.length > 0) {
                setFacultyDepartmentList(response.data);
            } else {
    ...
    const submitForm = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${NODE_API_URL}/api/faculty-department/register`, formData);
    ...
    const handleToggleStatus = async (dbId) => {
        ...
        try {
            ...
            const response = await dataFetchingPatch(`${NODE_API_URL}/api/faculty-department/status/${dbId}/${loguserid}/${login_type}`);
    ...
    const deleteStatus = async (dbId) => {
        ...
        try {
            ...
            if (deleteAlert) {
                const loguserid = secureLocalStorage.getItem('login_id');
                const login_type = secureLocalStorage.getItem('loginType');
                const response = await dataFetchingDelete(`${NODE_API_URL}/api/faculty-department/deleteStatus/${dbId}/${loguserid}/${login_type}`);
                if (response?.statusCode === 200) {
                    toast.success(response.message);
                    setTitleError('');
                    if (response.data == 1) {
    ...
    const updateDataFetch = async (dbId) => {
        if (!dbId || (!Number.isInteger(parseInt(dbId, 10)) || parseInt(dbId, 10) <= 0)) return toast.error("Invalid ID.");
        try {
            const response = await dataFetchingPost(`${NODE_API_URL}/api/faculty-department/fetch`,
                { dbId: dbId }
            );
    ...
    return (
        <>
                        ...
                        <div className="page-header mb-0">
                            <div className="header-sub-title">
                                <nav className="breadcrumb breadcrumb-dash">
                                    ...
                                    <span className="breadcrumb-item active">Faculty Department</span>
                                </nav>
                            </div>
                        </div>
                        <div className="card bg-transparent mb-2">
                            <div className="card-header d-flex justify-content-between align-items-center px-0">
                                <h5 className="card-title h6_new">Faculty Department List</h5>
            ...
            <Modal show={toggleShow} onHide={handleToggleShow}>
                <Modal.Header>
                    <Modal.Title>Faculty Department</Modal.Title>
                </Modal.Header>
                <Modal.Body>
