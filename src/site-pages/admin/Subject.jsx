...
function Subject() {
    ...
    const fetchList = async (deleteStatus = 0) => {
        ...
        try {
            const response = await dataFetchingPost(`${NODE_API_URL}/api/subject/fetch`,
                {
                    deleteStatus,
                    column: "id, subname, subcode, created_at, status, deleted_at, deleteStatus"
                });
    ...
    const submitForm = async (e) => {
        ...
        setTitleError('');
        if (!formData.subname) {
            setTitleError("Subject name is required.");
        ...
        try {
            ...
            const response = await axios.post(`${NODE_API_URL}/api/subject/register`, formData);
    ...
    const handleToggleStatus = async (dbId, currentStatus) => {
        ...
        try {
            ...
            const response = await dataFetchingPatch(`${NODE_API_URL}/api/subject/status/${dbId}/${loguserid}/${login_type}`);
    ...
    const deleteStatus = async (dbId) => {
        ...
        try {
            ...
            if (deleteAlert) {
                const loguserid = secureLocalStorage.getItem('login_id');
                const login_type = secureLocalStorage.getItem('loginType');
                const response = await dataFetchingDelete(`${NODE_API_URL}/api/subject/deleteStatus/${dbId}/${loguserid}/${login_type}`);
                if (response?.statusCode === 200) {
                    toast.success(response.message);
                    setTitleError('');
                    if (response.data == 1) {
        ...
        try {
            ...
            if (response?.statusCode === 200 && response.data.length > 0) {
                ...
                setFormData((prev) => ({
                    ...
                    subname: response.data[0].subname,
                    subcode: response.data[0].subcode,
                }));
                setToggleShow(!toggleShow);
            } else {
                toast.error("Data not found.");
                        ...
                        <div className="page-header mb-0">
                            <div className="header-sub-title">
                                <nav className="breadcrumb breadcrumb-dash">
                                    ...
                                    <span className="breadcrumb-item active">Subject</span>
                                </nav>
                            </div>
                        </div>
                        <div className="card bg-transparent mb-2">
                            <div className="card-header d-flex justify-content-between align-items-center px-0">
                                <h5 className="card-title h6_new">Subject List</h5>
            ...
            <Modal show={toggleShow} onHide={handleToggleShow}>
                ...
                <Modal.Body>
                    <form onSubmit={(e) => e.preventDefault()}>
                        <div className='row'>
                            <FormField label="Subject Name" name="subname" id="subname" borderError={titleError ? true : false} errorMessage={titleError} value={formData.subname} column='col-md-12' onChange={handleChange} />
                            <FormField label="Subject Code" name="subcode" id="subcode" borderError={titleError ? true : false} value={formData.subcode} column='col-md-12' onChange={handleChange} />
                        </div>
                    </form>
                </Modal.Body>
