...
function Registration() {
  ...
  useEffect(() => {
    if (RolePermission && RolePermission.length > 0) {
      if (!hasPermission("Critical Stock Products", "notify for restock")) {
        navigate("/forbidden");
      }
    }
  ...
  const ProductThresholdDataById = async (dbId) => {
    try {
      const response = await dataFetchingPost(
        `${NODE_API_URL}/api/inventory/product/threshold-product-data-by-id/`,
        { dbId }
      );
      if (response?.statusCode === 200 && response.data.length > 0) {
        setFormData((prev) => ({
  ...
  const fetchProductForUpdate = async (dbId) => {
    try {
      const response = await dataFetchingPost(
        `${NODE_API_URL}/api/inventory/product/fetch`,
        { dbId }
      );
      if (response?.statusCode === 200 && response.data.length > 0) {
        setFormData((prev) => ({
          ...
          <div className="container-fluid">
            ...
            <div className="card bg-transparent mb-2">
              <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                ...
                <div className="ml-auto id-mobile-go-back">
                  ...
                  {hasPermission("Product List", "list") && (
            ...
            <div className="card border-0">
              <div className="card-header d-flex justify-content-center align-items-center">
                ...
                {dbId && (
                  <Link
                    to={`/admin/inventory/product/threshold/raised-query/${pId}`}
                ...
                <div className={`table-responsive`}>
                  ...
                  >
                    <Column
                      ...
                      body={(rowData) =>
                        rowData.facultyName
                          ? validator.unescape(rowData.facultyName)
                          : "N/A"
                      }
                    />
                    <Column
                      ...
                      body={(rowData) =>
                        rowData.pname
                          ? validator.unescape(rowData.pname)
                          : "N/A"
                      }
                    />
                    <Column
                      ...
                      body={(rowData) => (
                        <>
                          {[0, 2].includes(rowData.status) && (
                            <Link
                              to={`/admin/inventory/product/threshold/raised-query/${pId}/${rowData.id}`}
                              className="btn btn-primary"
                              style={{ whiteSpace: "nowrap" }}
                            >
