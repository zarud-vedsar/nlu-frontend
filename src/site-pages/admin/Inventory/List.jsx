...
function TimeSlot() {
  ...
  useEffect(() => {
    if (RolePermission && RolePermission.length > 0) {
      if (!hasPermission("Inventory Category", "list")) {
        navigate("/forbidden");
      }
    }
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${NODE_API_URL}/api/inventory/category/create`,
        formData
      );
      if (
  ...
  const handleToggleStatus = async (dbId, currentStatus) => {
    ...
    try {
      ...
      const response = await dataFetchingPatch(
        `${NODE_API_URL}/api/inventory/category/status/${dbId}/${loguserid}/${login_type}`
      );
  ...
  return (
    <>
            ...
            <div className="card bg-transparent mb-2">
              <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                ...
                <div className="ml-auto id-mobile-go-back">
                  ...
                  {hasPermission("Inventory Category", "create") && (
                    <button
                    ...
                    <Column
                      ...
                      body={(rowData) => (
                        <div className="d-flex">
                          {hasPermission("Inventory Category", "status") && (
                          ...
                          {hasPermission("Inventory Category", "update") && (
                            <div
      ...
      <Modal show={toggleShow} onHide={handleToggleShow}>
        <Modal.Header>
          <Modal.Title>
            {formData.dbId ? "Update Category" : "Add New Category"}{" "}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <form onSubmit={(e) => e.preventDefault()}>
            <div className="row">
              <FormField
                ...
                name="ctitle"
                id="ctitle"
                borderError={titleError ? true : false}
