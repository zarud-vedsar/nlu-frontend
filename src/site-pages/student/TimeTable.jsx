...
function TimeTableChartPrint() {
  ...
  const courseListDropdown = async () => {
    try {
      const response = await axios.post(
        `${NODE_API_URL}/api/course-selection/fetchCurrentCourseActive`,
        {
          sid,
        }
      );
  ...
  const ftechChartList = async (courseId, semesterId) => {
    try {
      const response = await axios.get(
        `${NODE_API_URL}/api/time-table/table-chart/chart-complete-by-course-semester/${courseId}/${semesterId}`
      );
      if (response.data?.statusCode === 200 && response.data.data.length > 0) {
  ...
  useEffect(() => {
    if (
      ...
      CurrentCourse.course.length > 0 &&
      CurrentCourse.course[0].courseid &&
      CurrentCourse.course[0].semesterid
    ) {
      // Call the function only when both IDs exist
      ftechChartList(
        CurrentCourse.course[0].courseid,
        CurrentCourse.course[0].semesterid
      );
    }
  }, [CurrentCourse, timeChartId]);
