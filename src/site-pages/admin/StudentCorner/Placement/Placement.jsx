...
function MyVerticallyCenteredModal(props) {
  ...
  useEffect(()=>{
    const decode = async ()=>{
      await decodeHtml(props?.selectedPlacement?.description).then((res)=>{
        setDescription(res);
      })
    };
    decode()
  },[props?.selectedPlacement?.description])
  return (
    <Modal
      {...props}
      size="lg"
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">
          {props?.selectedPlacement?.position}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
       {description &&  <div
...
const Placement = () => {
  ...
  const viewAllImages = (index) => {
    const currentLob = placementList[index];
    setSelectedPlacement(currentLob);
  };
  useEffect(() => {
  ...
  const showRecyleBin = () => {
    ...
    recycleTitle === "Show Recycle Bin"
      ? getDeletedPlacementList()
      : loadPlacementsData();
  };

  const getDeletedPlacementList = async () => {
    setLoading(true);
    try {
  ...
  const loadPlacementsData = async (filter = {}) => {
    setLoading(true);
    try {
      const bformData = new FormData();
      bformData.append("data", "load_placement");
  ...
  const editDetail = (id) => {
    navigate(`/admin/edit-placement/${id}`);
  };
  ...
  const deletePlacement = async (id) => {
    try {
      ...
      bformData.append("data", "delete_placement");
  ...
  const updateStatus = async (id) => {
    try {
      ...
      if (response.status == "200") {
        const updatedPlacements = placementList?.map((placement) =>
          placement.id === id
        ...
        setPlacementList(updatedPlacements);
      }
    } catch (error) {
      const status = error.response?.data?.status;
        ...
        <div className="container-fluid">
          ...
          <div className="card bg-transparent mb-2">
              <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                <h5 className="card-title h6_new">Placement List</h5>
                            ...
                            >
                              <div className="avatar ml-2 avatar-icon avatar-md avatar-red">
                                <i
                                  ...
                                  onClick={() => deletePlacement(rowData.id)}
                                ></i>
                              </div>
                            </OverlayTrigger>
                          ) : (
