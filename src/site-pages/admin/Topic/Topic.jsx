...
function Topic() {
  ...
  const fetchTopicListing = async (filters = {}, deleteStatus = 0) => {
    setIsFetching(true);
    const { status, courseid, semesterid, subject, topicid } = filters;
    try {
      const response = await dataFetchingPost(
        `${NODE_API_URL}/api/topic/fetch`,
        {
  ...
  const showRecyleBin = () => {
    ...
    resetFilters();
    fetchTopicListing({}, recycleTitle === "Show Recycle Bin" ? 1 : 0);
  };
  ...
  const handleToggleStatus = async (dbId, currentStatus) => {
    ...
    try {
      ...
      const response = await dataFetchingPatch(
        `${NODE_API_URL}/api/topic/status/${dbId}/${loguserid}/${login_type}`
      );
    ...
    try {
      ...
      if (deleteAlert) {
        ...
        const response = await dataFetchingDelete(
          `${NODE_API_URL}/api/topic/deleteStatus/${dbId}/${loguserid}/${login_type}`
        );
        if (response?.statusCode === 200) {
          toast.success(response.message);
          if (response.data == 1) {
  ...
  const updateDataFetch = async (dbId) => {
    if (
      ...
      return toast.error("Invalid ID.");
    navigate(`/admin/edit-topic/${dbId}`, { replace: false });
  };
          ...
          <div className="container-fluid">
            ...
            <div className="card bg-transparent mb-2">
              <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                <h5 className="card-title h6_new">Topic List</h5>
