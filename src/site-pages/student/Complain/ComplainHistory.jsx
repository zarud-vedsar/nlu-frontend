...
function ComplainHistory() {
  const [raisedComplains, setRaisedComplains] = useState([]);
  ...
  const viewMessage = (index) => {
    const currentQuery = raisedComplains[index];
  ...
  const handleSubmit = async (e = false) => {
    ...
    try {
      const response = await axios.post(
        `${NODE_API_URL}/api/hostel-management/student/room-complaints-by-student-list`,
        {
          ...bformData,
        }
      );
      if (response.data?.statusCode === 200 && response.data.data.length > 0) {
        setRaisedComplains(response.data.data);
      } else {
        setRaisedComplains([]);
      }
      setIsFetching(false);
          ...
          <div className="container-fluid">
            ...
            <div className="card bg-transparent mb-2">
              <div className="card-header d-flex justify-content-between align-items-center px-0">
                <h5 className="card-title h6_new">Complain History</h5>
