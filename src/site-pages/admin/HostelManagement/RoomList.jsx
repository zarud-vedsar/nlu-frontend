...
import { dataFetchingDelete, dataFetchingPatch, formatDate, goBack } from '../../../site-components/Helper/HelperFunction';
...
function RoomList() {
    ...
    const [RoomList, setRoomListing] = useState([]);
    ...
    const handleToggleStatus = async (dbId, currentStatus) => {
        ...
        try {
            ...
            const response = await dataFetchingPatch(`${NODE_API_URL}/api/hostel-management/room/status/${dbId}/${loguserid}/${login_type}`);
            if (response?.statusCode === 200) {
                toast.success(response.message);
                // Update the notice list to reflect the status change
                setRoomListing(prevList =>
                    prevList.map(item =>
    ...
    const handleSubmit = async (e = false, deleteStatus = 0) => {
        if (e) e.preventDefault();
        setIsFetching(true);
        try {
            const response = await axios.post(`${NODE_API_URL}/api/hostel-management/room/fetch`, {
                deleteStatus: deleteStatus,
                status: formData.status
            });
            if (response.data?.statusCode === 200 && response.data.data.length > 0) {
                setRoomListing(response.data.data);
            } else {
                setRoomListing([]);
            }
            setIsFetching(false);
    ...
    const deleteStatus = async (dbId) => {
        ...
        try {
            ...
            if (deleteAlert) {
                const loguserid = secureLocalStorage.getItem('login_id');
                const login_type = secureLocalStorage.getItem('loginType');
                const response = await dataFetchingDelete(`${NODE_API_URL}/api/hostel-management/room/deleteStatus/${dbId}/${loguserid}/${login_type}`);
                if (response?.statusCode === 200) {
                    toast.success(response.message);
                    setRoomListing(prevList => prevList.filter(item => item.id !== dbId));
                } else {
                    toast.error("An error occurred. Please try again.");
                }
            }
    ...
    const showRecyleBin = () => {
        ...
        handleSubmit(false, recycleTitle === "Trash" ? 1 : 0);
    }
    return (
        <>
                            ...
                            <div className='card-body'>
                                ...
                                <div className={`table-responsive ${isFetching ? 'form' : ''}`}>
                                    ...
                                    >
                                        ...
                                        <Column field="occupied_beds" header="Occupied Beds" sortable />
                                        <Column field="roomType" header="Room Type" sortable />
