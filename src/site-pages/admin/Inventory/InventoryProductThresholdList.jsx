...
function ProductList() {
  ...
  useEffect(() => {
    if (RolePermission && RolePermission.length > 0) {
      if (!hasPermission("Critical Stock Products", "list")) {
        navigate("/forbidden");
      }
    }
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await dataFetchingPost(
        `${NODE_API_URL}/api/inventory/product/threshold-products`,
        {
  ...
  const handleToggleStatus = async (dbId, currentStatus) => {
    ...
    try {
      ...
      const response = await dataFetchingPatch(
        `${NODE_API_URL}/api/inventory/product/status/${dbId}/${loguserid}/${login_type}`
      );
    ...
    try {
      ...
      if (deleteAlert) {
        ...
        const response = await dataFetchingDelete(
          `${NODE_API_URL}/api/inventory/product/deleteStatus/${dbId}/${loguserid}/${login_type}`
        );
        if (response?.statusCode === 200) {
          toast.success(response.message);
          if (response.data == 1) {
          ...
          <div className="container-fluid">
              ...
              <div className="header-sub-title">
                <nav className="breadcrumb breadcrumb-dash">
                  ...
                  <span className="breadcrumb-item active">
                    Inventory Alert: Limited Stock Items
                  </span>
                </nav>
              </div>
            </div>
            <div className="card border-0 bg-transparent mb-2">
              <div className="card-header border-0 bg-transparent py-1 id-pc-divices-header px-0 id-mobile-divice-d-block">
                <h5 className="card-title h6_new pt-0">
                  Inventory Alert: Limited Stock Items
                </h5>
                <div className="ml-auto id-mobile-go-back">
                  <button
                    className=" btn border-0 goback mr-2"
                    onClick={() => goBack()}
                  >
                ...
                {hasPermission("Add Product","create") && (
                  <button
                    onClick={() =>
                      navigate("/admin/inventory/add-product", {
                        replace: false,
                      })
                    }
                ...
                <div className={`table-responsive ${isFetching ? "form" : ""}`}>
                  ...
                  >
                    <Column
                      ...
                      header="Category"
                      sortable
                      body={(rowData) => validator.unescape(rowData.ctitle)}
                    />
                    <Column
                      field="pname"
                      header="Product"
                    ...
                    <Column
                      ...
                      header="Unit"
                      sortable
                      body={(rowData) => validator.unescape(rowData.punit)}
                    />
                    <Column
                      field="pbrand"
                      header="Brand"
                    ...
                    <Column
                      ...
                      sortable
                      body={(rowData) => rowData.threshhold_limit}
                    />
                    <Column
                      header="Action"
                      body={(rowData) => (
                        <>
                        {hasPermission("Critical Stock Products","notify for restock") && (
                          <Link
                            to={`/admin/inventory/product/threshold/raised-query/${rowData.id}`}
