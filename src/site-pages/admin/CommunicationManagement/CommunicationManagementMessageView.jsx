...
function MessageList() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${NODE_API_URL}/api/communication/reply-message-by-faculty`, {
                dbId,
                        ...
                        <div className="card bg-transparent mb-2">
                            <div className="card-header id-pc-divices-header px-0 id-mobile-divice-d-block">
                                ...
                                <div className="ml-auto id-mobile-go-back">
                                     <button
                                                       className="mr-auto  btn go-back  mr-2"
                                                       onClick={goBack}
                                                     >
                                                                        ...
                                                                        item.sentBy == 'student' && (
                                                                            <div className="bubble">
                                                                                <div className="bubble-wrapper mb-0">
                                                                                    <span>{item?.message ? validator.unescape(item.message) : ''}</span>
                                                                                </div>
                                                                                <span>{item?.sname}</span> <span className="font-10">{formatDate(item.created_at)}</span>
                                                                            </div>
                                                                        )
                                                                    }
                                                                </div>
                                                                <div className="msg msg-sent">
                                                                    {
                                                                        item.sentBy == 'faculty' && (
                                                                            <div className="bubble">
                                                                                ...
                                                                                <span>{item?.facultyName}</span> <span className="font-10">{formatDate(item.created_at)}</span>
                                                                            </div>
                                                                        )
                                                                    }
                                                                </div>
                                                                ...
                                                                <div className="msg msg-recipient">
                                                                    {
                                                                        item.sentBy == 'student' && (
                                                                            <div className="bubble">
                                                                                ...
                                                                                <span className="font-10">{item?.sname}</span> <span className="font-10">{formatDate(item.created_at)}</span>
                                                                            </div>
                                                                        )
                                                                    }
                                                                </div>
