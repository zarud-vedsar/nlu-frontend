...
function InventoryReport() {
  ...
  const publisherData = async () => {
    try {
      const bformData = new FormData();
      bformData.append("data", "book_publishers");
  ...
  const authorsData = async () => {
    try {
      const bformData = new FormData();
      bformData.append("data", "book_authors");
      const response = await axios.post(
        `${PHP_API_URL}/report.php`,
        bformData,
        {
  ...
  const languageData = async () => {
    try {
      const bformData = new FormData();
      bformData.append("data", "language");
      const response = await axios.post(
        `${PHP_API_URL}/report.php`,
        bformData,
        {
  ...
  const exportExcelFile = async (e) => {
    ...
    try {
      ...
      const formDataEntries = [
        ...
        { name: "data", value: "lib_books" },
        { name: "publisher", value: formData?.publisher },
                ...
                <form>
                  <div className="row">
                    <div className="col-md-4 col-lg-4 col-12 form-group">
                      <label className="font-weight-semibold">Publisher</label>
                      <Select
                        ...
                        onChange={(selectedOption) => {
                          setFormData({
                            ...formData,
                            publisher: selectedOption ? selectedOption.value : "", // Update state with selected value or empty string
                          });
                        }}
                        value={
                          formData.publisher
                            ? { value: formData.publisher, label: formData.publisher } // Match selected value and label
                            : null // Set `null` if no value is selected
                        }
                      />
                    </div>
                    <div className="col-md-4 col-lg-4 col-12 form-group">
                      <label className="font-weight-semibold">Author</label>
                      <Select
                        ...
                        onChange={(selectedOption) => {
                          setFormData({
                            ...formData,
                            author: selectedOption ? selectedOption.value : "", // Update state with selected value or empty string
                          });
                        }}
                        value={
                          formData.author
                            ? { value: formData.author, label: formData.author } // Match selected value and label
                            : null // Set `null` if no value is selected
                        }
                      />
                    </div>
                    <div className="col-md-4 col-lg-4 col-12 form-group">
                      ...
                      <Select
                        ...
                        onChange={(selectedOption) => {
                          setFormData({
                            ...formData,
                            language: selectedOption ? selectedOption.value : "", // Update state with selected value or empty string
                          });
                        }}
                        value={
                          formData.language
                            ? { value: formData.language, label: formData.language } // Match selected value and label
                            : null // Set `null` if no value is selected
                        }
                      />
                    </div>
