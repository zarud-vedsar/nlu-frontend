...
function MultipleChoiceQuestion() {
  ...
  const toggleAnswer = (selectedOption) => {
    setQuestionList((prevList) => {
      return prevList.map((question, index) => {
        if (index === currentQuestion) {
          ...
          const updatedAnswers = selectedAnswers.includes(selectedOption)
            ? selectedAnswers.filter((opt) => opt !== selectedOption) // Remove if already selected
            : [...selectedAnswers, selectedOption]; // Add if not selected

          // Determine attempted state
          const attempted = updatedAnswers.length > 0 ? 1 : 0;

          return {
            ...question,
            answer: updatedAnswers,
            attempted, // Adding attempted as part of the object
          };
        }
        return question;
      });
        ...
        <div className="container-fluid ">
          ...
          <div className="row">
            <div className="col-12 col-sm-12 col-md-8 col-lg-8">
              <div
                className="card"
                style={{ minHeight: "70vh", border: "2px solid #b0b0b0" }}
              >
                <div className="card-body">
                  ...
                  <div className="row">
                    {["option1", "option2", "option3", "option4"].map(
                      (option, index) => (
                        <div className="col-12" key={option}>
                          <input
                            type="checkbox"
                            id={option}
                            className="mr-1"
                            checked={questionList[
                              currentQuestion
                            ]?.answer?.includes(option)}
                            onChange={() => toggleAnswer(option)}
                          />
                          <label htmlFor={option}>
                            {questionList[currentQuestion]?.[option]}
                          </label>
                        </div>
                      )
                    )}
                  ...
                  <div className="row">
                    <div className="col-12">
                      <input
                        ...
                        checked={questionList[
                          currentQuestion
                        ]?.answer?.includes("option5")}
                        onChange={() => toggleAnswer("option5")}
                      />
