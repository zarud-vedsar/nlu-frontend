 ...
 */
  ...
  useEffect(() => {
    if (RolePermission && RolePermission.length > 0) {
      const showSubMenu = hasPermission("Assignment", "list");
      if (!showSubMenu) {
        navigate("/forbidden");
      }
    }
  ...
  const [assignmentListing, setAssignmentListing] = useState([]);
    ...
    try {
      const response = await dataFetchingPost(
        `${NODE_API_URL}/api/assignment/fetch`,
        {
          ...
          semesterid: semesterid,
          subject: subject,
          column: "id, assignment_title",
        }
      );
      if (response?.statusCode === 200 && response.data.length > 0) {
        setAssignmentListing(response.data);
      } else {
  ...
  const showRecyleBin = () => {
    ...
    resetFilters();
    fetchAssignmentListing({}, recycleTitle === "Show Recycle Bin" ? 1 : 0);
  };
  ...
  const handleToggleStatus = async (dbId, currentStatus) => {
    ...
    try {
      ...
      const response = await dataFetchingPatch(
        `${NODE_API_URL}/api/assignment/status/${dbId}/${loguserid}/${login_type}`
      );
    ...
    try {
      ...
      if (deleteAlert) {
        ...
        const response = await dataFetchingDelete(
          `${NODE_API_URL}/api/assignment/deleteStatus/${dbId}/${loguserid}/${login_type}`
        );
        if (response?.statusCode === 200) {
          toast.success(response.message);
          if (response.data == 1) {
            fetchAssignmentListing(filters, 1);
          } else {
            fetchAssignmentListing(filters, 0);
          }
          showRecyleBin();
        } else {
          toast.error("An error occurred. Please try again.");
  ...
  const updateDataFetch = async (dbId) => {
    if (
      ...
      return toast.error("Invalid ID.");
    navigate(`/admin/edit-assignment/${dbId}`, { replace: false });
  };
              ...
              <div className="card-body">
                ...
                <div className={`table-responsive ${isFetching ? "form" : ""}`}>
                  ...
                  >
                    ...
                    <Column
                      field="deadline_date"
                      body={(row) => formatDate(row.deadline_date)}
                    ...
                    {
                      hasPermission("Assignment", "add question") && (
                        <Column
                          body={(row) => (
                            <Link
                              to={`/admin/assignment/add-question/${row.id}`}
                    ...
                    <Column
                      ...
                      body={(rowData) => (
                        <div className="d-flex">
                          {
                            hasPermission("Assignment", "status") && (
            ...
            <div className="col-md-12 col-12 form-group">
              ...
              <Col md={12} className="mb-3">
                <Form.Group controlId="status">
                  ...
                  <Select
                    options={assignmentListing.map((item) => ({
                      value: item.id,
                      label: capitalizeFirstLetter(item.assignment_title),
                    }))}
                    onChange={(selectedOption) => {
                      setFilters({
                        ...filters,
                        assignmentid: selectedOption.value,
                      });
                    }}
                    value={
                      assignmentListing.find(
                        (item) => item.id === filters.assignmentid
                      )
                        ? {
                        ...
                        : { value: filters.assignmentid, label: "Select" }
                    }
                  />
                </Form.Group>
              </Col>
